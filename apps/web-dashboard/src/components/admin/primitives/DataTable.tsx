...
/**
 * Generic sortable, paginated data table. Supporting both rows/data,
 * rowKey/keyExtractor, and accessor/render for robust backwards-compatibility.
 */
export function DataTable<T>({
  ...
  const getRowKey = (row: T, idx: number): string => {
    ...
    if (keyExtractor) return String(keyExtractor(row, idx));
    return String((row as any)?.id || (row as any)?.code || idx);
  };
  ...
  const sortedRows = useMemo(() => {
    ...
    if (!col?.sortValue) return tableRows;
    ...
    return copy;
  }, [tableRows, sortKey, sortDir, columns]);
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-[var(--adm-border)]">
            {pageRows.map((row, idx) => (
              <tr key={getRowKey(row, idx)} className="hover:bg-[var(--adm-primary-wash)] transition-colors">
                {columns.map((col) => {
